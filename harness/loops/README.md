...
`escalate_on.new_findings_not_falling`, `escalate_on.unrefereed_fix` and
...
| `review_panel.file_deferral_issues` | Which deferrals get a GitHub ISSUE as well as the board row every deferral gets anyway (#482). **`P2`**. The two were being treated as one record and they are not: the row chains by finding key across rounds, feeds `/panel` and keeps a confident wrong finding from scoring like a real one, while the issue is a work item on somebody's tracker. Those coincide for a P1 or P2 deferral and do not for the P3/P4 tail, which is where the volume is — measured on this repo on 2026-08-26, roughly twenty open issues were panel deferred-finding exhaust and nothing else (#66 #69 #72 #74 #95 #104 #111 #119 #120 #126 #132 #133 #140 #223 #237 #285 #286 #288 #300), and #283 is a rescue *from* one of them. At or above the gate the orchestrator opens the issue and names it in `deferred_to`; below it the row carries **no `deferred_to`** (the column is nullable, the API accepts a targetless `deferred`, and `/panel` renders it rather than breaking) and **a one-line `note`** instead — which is what makes the row worth reading later, and `GET /review/findings?repo=&pr=` is the read that write exists for. `always` is the pre-#482 behaviour and `never` files none. **An escalation is exempt at every setting**, `never` included: its issue asks a question rather than filing a task, and it is what carries that question past the end of the session. **An unverifiable claim is exempt on the same reasoning** (#547) and is the one road here with no board row at all — it is not a finding, so nothing on the board keys it, and its durable record is the round's payload plus the issue. If the board write fails the orchestrator files the issue whatever the gate says — below the gate the row is the only record, so losing both loses the finding. The report's dial line says the answer in words on every round. |
...
- **No settled CI result is its own veto, and it is not the same as nothing read**
  ...
  in **their own sentence**, and the wording matters because only three of them are
  claims about execution — `none` (nothing ran), `blocked` (#324: a run exists,
  is gated on a person and has executed nothing) and #548's `local-fail` (something
  ran here and went red) — while `PENDING` is #501's
  residue after the bounded wait and may hold green checks, `unknown` is a
  lookup that failed and says nothing either way, and `local-unknown` is a command
  that was started and did not report. Could-not-check is not
  nothing-to-report. #548's `local-pass` is the one state argued INTO the
  non-vetoing set since, and the section below has the argument — including why
  `local-fail` is not, which is the only place a local answer and a CI answer of
  the same shape are treated differently.
...
**It fires only where GitHub was silent** — `none`, `blocked`, `unknown`. A real CI
result about this exact commit is never displaced by a weaker local one, and
`PENDING` belongs to #501's bounded wait, which is in the business of turning it
into a real one.

**It runs only in a checkout already sitting on the PR's head, with no uncommitted
edits to tracked files** (`panel_scope._local_head_problem`) — and that guard is the
reason the setting is safe to have at all. `panel.py --repo x --pr N` reviews a PR
without ever checking it out; a feature that read a command from one branch and ran
it over another would turn a review into an execution channel for a PR nobody has
read. Where the checkout *is* the PR's head, whoever put it there has already
accepted this code on this box and running its tests adds nothing `git checkout` did
not — which is exactly the fix loop's own worktree, the case this exists to serve.
Untracked files are tolerated (a provisioned worktree carries a `.env`, a virtualenv
and scratch of its own); that boundary is stated rather than silent, and a repo that
cannot accept it should not declare a suite.
...
`local-unknown` vetoes too, in its own sentence: a command was started and told us
nothing, which is not the same claim as `none`'s "nothing mechanical executed this
code".
...
the operator's terminal and the payload and nowhere else. The commands, their timing
...
round. What is bounded is memory and wall clock; **disk is not**, and a repo whose own
declared command can fill one has a problem the panel is not the place to solve.
