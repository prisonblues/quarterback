...
`low_severity_fix_pct`, `unrefereed_line_weight`,
...
| `review_panel.low_severity_fix_pct` | The **proportional** half of that budget (#551): the most of the cycle's **first round** the same band may spend, as a percentage, with the round spending whichever half is **smaller**. **22.4**; `null` switches this half off and is the exact pre-#551 behaviour. Both halves are ceilings, so the pair can only ever **tighten**. **The opposite operator to `min_fix_growth_chars`, and the symmetry argument gets it backwards.** `max_fix_growth`'s defect is that a proportional allowance shrinks with the PR while diff framing does not, so a tiny PR cannot afford one honest fix — a **floor**. What this budget measures is accumulation **relative to the change** (#297: #188's 185-line feature became 721 churned lines, 74% of it review-response code), so its dangerous end is the **small** PR, where a fixed 40 lines can exceed the diff it is polishing — 40 is ~22% of a 179-line change and larger than a 30-line one — while on a 5,000-line PR 40 lines of P3 work is harmless noise. Anyone implementing this from "40 lines is noise on a big PR" writes a `max`, and that is the version that reintroduces #188. **22.4 is the allowance the absolute already grants**: 40 / 179 = 22.35%, measured on the one run this dial has been watched on — a 179-line PR that spent 31 of its 40 lines and dropped a 17-line fix against 9 remaining, which is the dial working rather than failing. The crossover is **178.6 churned lines**: at or above it the absolute binds and nothing changes; below it the proportion does. Rounded **up** from 22.35 so that run keeps all 40 lines and the key is a provable no-op on every run anybody has measured. A 30-line PR gets 6, a 100-line one 22, a 500-line one the full 40. **The conversion is re-measured and does not agree with the 66 written beside `max_fix_growth_chars`**: a baseline records the first round only as `pr_chars`, so `panel_seats.CHARS_PER_CHURNED_LINE` (**58**) turns it into the lines this budget is spent in — PR #188's own merged diff, 34,635 chars over 597 churned lines. 34,717 reproduces on today's PR and 521 does not (nor does #297's 721), so both line counts look to predate a rebase; 66 is left where it is rather than corrected on a guess. Cross-checked against this repo's last 25 non-merge commits (60-95 chars a churned line, median 81), and **58 sits at the bottom of that on purpose** — under-stating chars-per-line over-states the PR's line count and so **loosens** this ceiling, and #664 has just measured what a budget that binds too early costs. **A derived budget is never zero**: `low_severity_fix_lines: 0` is a written value that raises the applied floor to the cut, so a proportion flooring to 0 would take that decision on an operator's behalf as a side effect of the PR's size. The derived value is clamped at **1**, a written `0` is left alone, and this key may lower a budget but never raise one or switch one off. A percentage beside `low_severity_fix_lines: null` bounds nothing and gets a `config_notes` line rather than silently becoming a budget. `0` and anything above `100` are refused — the second because a proportion over 100 permits a round to spend more lines than the change contains, which is the shape #551 was filed about. **The shape is a human's decision** (#551 is `needs-human/decision`). Rejected: a two-part `low_severity_fix_lines` (a bare `null` would have to say which half it switches off, on a key where `null` and `0` already differ); a folded lines-per-char dial, which is one number rather than two but hides which half is the policy; a churned-line count on the baseline, which needs no rate and would decline to run on every cycle in flight — #492's own objection to a `_lines` dial; and any change to cheapest-first ordering, which #551 rules out because a value term hands back the judgement #297 removed. |
...
`max_fix_growth`, `max_fix_growth_chars`, `min_fix_growth_chars`, `low_severity_fix_pct` and `low_severity_fix_lines`, all of which distinguish an absent key
