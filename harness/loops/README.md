...
`low_severity_fix_full_chars`, `unrefereed_line_weight`,
`max_fix_growth`, `max_fix_growth_chars`, `min_fix_growth_chars`, `max_fix_guard_lines`,
...
| `review_panel.low_severity_fix_full_chars` | The **proportional** half of that budget (#551), written as a **size** rather than a rate: the first round's `pr_chars` at or above which the whole `low_severity_fix_lines` budget applies, with anything smaller getting it **pro rata** (`lines × first_chars ÷ this`) and the round spending whichever half is **smaller**. **14,325**; `null` switches this half off and is the exact pre-#551 behaviour. Both halves are ceilings, so the pair can only ever **tighten**. **The opposite operator to `min_fix_growth_chars`, and the symmetry argument gets it backwards.** `max_fix_growth`'s defect is that a proportional allowance shrinks with the PR while diff framing does not, so a tiny PR cannot afford one honest fix — a **floor**. What this budget measures is accumulation **relative to the change** (#297: #188's feature became 721 churned lines, 74% of it review-response code), so its dangerous end is the **small** PR, where a fixed 40 lines can exceed the diff it is polishing, while on a 5,000-line PR 40 lines of P3 work is harmless noise. Anyone implementing this from "40 lines is noise on a big PR" writes a `max`, and that is the version that reintroduces #188. **Chars, and that is a correctness point rather than a style one.** The budget is counted in churned lines and the only first-round size a baseline records is `pr_chars`, so this had to be written in one unit or the other. In chars it states its crossing in the unit the code compares and needs no chars-per-line rate **at run time** — the arithmetic is a ratio of two char counts multiplying a line count, so the units cancel. An earlier draft was a percentage over a `CHARS_PER_CHURNED_LINE` constant; both are gone. **14,325 is measured in chars rather than converted into them.** #551's policy is a ratio — "on a 179-line PR, 40 lines is a sane ~22% allowance" — putting the full budget at ~182 churned lines. Rather than multiply by an assumed rate, every merged PR of this repo with churn between 120 and 260 lines (**n=21**) was fetched, measured in chars and scaled to exactly 182 churned lines: **median 14,325**, range 9,538-18,604. It is the size PRs of that shape actually are, it is re-runnable, and the spread is stated rather than hidden (n=21 supports a median, not a fourth significant figure). **What that is and is not free of, narrowly:** the run-time arithmetic is rate-free unqualified, but the calibration anchors on 182 **churned lines** and scales each PR to that count, so the line framing moved out of the formula and into the number. It is free of the *disputed 66-versus-58 rate* — every PR contributed its own measured chars and its own measured churn rather than one assumed rate standing in for all of them — and it still rests on churn being counted the same way across this repo, which is exactly what #692 is open about, PR #188 carrying three different line counts (521, 597, 721) in three places. Less exposed, not immune. In the dial's own unit: 40,000 chars buys the full 40 lines, 10,000 buys 27, 5,000 buys 13, 2,000 buys 5. **Clamped at one honest one-line fix.** Pro rata alone reaches 1 line at 359 chars and 3 at 1,075, and a budget of one or two lines is not a small budget — it is "fix nothing" written so it reads like a budget. `git diff --numstat` reports **aggregate** insertions and deletions per file and has no notion of a changed line; what is true is that a one-line replacement commonly arrives as one deletion plus one insertion (verified), and `_referee_kind_lines` counts every `-` and `+` alike. So the cheapest correction to an existing line commonly costs 2, and the commonest below-floor fix is a comment or stale docstring, which `unrefereed_line_weight` prices at 2 apiece. The clamp is `2 × unrefereed_line_weight` — **4** at the shipped weight — moving with the weight because the weight is the unit the budget is counted in. A first round **measured at zero** chars gets that clamp and not the whole budget: unknown and zero are different claims, and the smallest measurement must not buy the largest budget. **Since #674 a starved budget is expensive rather than untidy**: an unpayable fix is declared `--declined <key>:budget`, that appends a veto, the veto costs the round its `stop_confident`, and `preland --require-earned-stop` turns a false `stop_confident` into a failed check — so a budget too small to buy anything holds the PR out of a strict landing for the rest of the cycle. Below **1,791** chars the clamp *is* the budget — simply where the pro-rata share first rises above it, at 5, and **not** where a second such fix becomes affordable, which is 2,865. A stated concession, made in the direction #674 prices. **It can never raise a budget.** The clamp sits **inside** `min(written, …)`, so a repo at `low_severity_fix_lines: 3` gets 3 and not 4, and a written `0` — which raises the applied floor to the cut — is left alone. A size beside `low_severity_fix_lines: null` bounds nothing and gets a `config_notes` line rather than silently becoming a budget. `0` is refused: a size every PR is at or above is the off position in a spelling that does not say so. **The shape is a human's decision** (#551 is `needs-human/decision`). Rejected: a two-part `low_severity_fix_lines` (a bare `null` would have to say which half it switches off, on a key where `null` and `0` already differ); a percentage with a chars-per-line constant, which stated its crossing in a unit the code does not compare and inherited #692; a churned-line count on the baseline, which needs no size and would decline to run on every cycle in flight — #492's own objection to a `_lines` dial; and any change to cheapest-first ordering, which #551 rules out because a value term hands back the judgement #297 removed. |
...
| `review_panel.max_fix_growth_chars` | The ABSOLUTE half of that same ceiling (#492): chars the PR may **grow** past the size the cycle's first round read it at, with the cycle stopping on whichever of the two is crossed **first**. **30,000**; `null` switches this half off and leaves the multiple, and `null` on both is no growth check at all. A pure multiple hands its rope out in proportion to the starting size — at 3.0x a 113-line PR may grow ~226 lines and a 2,000-line one may grow 4,000, so the loosest allowance goes to the case most in need of a ceiling. Chars rather than lines because the multiple beside it already divides `pr_chars` by `pr_chars`, and two halves of one ceiling read off two different measurements is #298's defect one level up; 30,000 is ~380-450 churned lines at PR #188's own measured 66 chars a line, which stops #188's +536 and #236's +1,954 with margin. Both are ceilings, so **that pair** can only ever **tighten** — nothing the arrangement lets through would have been caught by the multiple alone. Narrow claim: it is about these two keys and not about the growth check, which since #664 has a floor in it that loosens. A repo that wrote `max_fix_growth: null` before #492 gets a `config_notes` line saying the other half is still in force and naming the key that finishes the job. |
| `review_panel.min_fix_growth_chars` | The **floor** under the multiple (#664), and the one dial in this block that **loosens**: the ratio half fires only where the PR has **also** grown by more than this many chars. **2,000**; `null` switches the floor off and is the exact pre-#664 behaviour. The absolute half is untouched by it, so a PR of any size that grows past `max_fix_growth_chars` stops exactly as before. **Proportionality has two ends and #492 only fixed the top.** Diff framing is fixed per file-hunk — `diff --git`, `index`, `---`/`+++`, `@@` and a hunk's context lines run to ~430 chars before one character of the actual repair — while the multiple's allowance shrinks with the PR. On the 439-char PR this was measured from, the whole 3.0x allowance was 878 chars and the smallest honest single-hunk fix cost 827 of it, 49% of the allowance going on the shape of a diff; a second finding whose fix was ~200 chars could not be paid for at all. The breakevens: the allowance equals one hunk's framing at a PR of **215** chars and equals that measured minimum hunk at **413.5**, so below ~413 chars a 3.0x ceiling cannot afford one truthful one-file fix and the dial stops meaning "a fix pass that multiplies the diff has written a second change". **And it wrote the regression it charged for**: the fixer on that cycle declared two corrections it could not pay for and the next round found one of them, so the ceiling caused the defect and then bought a round to rediscover it. **2,000 is a two-file fix** — ~860 of framing plus ~1,140 of content, ~17 churned lines at PR #188's measured 66 chars a line — and it sits far under 30,000 on purpose: the floor guards the **ratio only**, so a floor near the absolute ceiling would leave the multiple unable to fire first and crossed-first would have inverted. A repo writing one at or above `max_fix_growth_chars` gets a `config_notes` line saying the multiple can no longer bind first. `0` is refused — a floor every growth clears is the off position in a spelling that does not say so. **The shape is a human's decision** (#664 is `needs-human/decision`): the alternative is measuring both ends **net of framing**, which measures what the dial claims to measure and needs no key at all, and is rejected because `pr_chars` on every baseline in flight is a raw char count — a framing-net numerator would decline to run on every cycle in flight and every payload behind it, which is #169's ships-unwired failure and the same objection #492 used to reject a `_lines` dial. #551 is this question from the other end and this key does **not** settle it: what `low_severity_fix_lines` wants is a **ceiling**, `min(lines, pct × the first round)`, because the accumulation it measures is dangerous on the small PR — same question, opposite operator. |
...
| `review_panel.escalate_on` | #78's reserved matters — the decisions the process may not take on its own. Four are built, and the two above are the volume/attribution pair. `premise_repeated` (**2**) is the OCCURRENCE a declared premise is stopped on: the second time a fix is written against one premise, stop. `premise_undecidable` (**true**) stops on the FIRST declaration that answers `--premise-decidable no` — the property the fix asserts is not observable in the runtime the assertion runs in, so every fix for it is an approximation and no number of rounds converges (#491). The asymmetry is the point: one counts because a single declaration is not evidence, the other reads a fact about the runtime that a repeat cannot make truer. It exists because the counter is structurally blind to proxy-replacement — a fixer swapping one proxy for a better one declares a genuinely different premise each round, and one measured cycle declared four that no two of matched. `quorum_failed` and `judge_absent` are accepted, reported in `config_notes` and enforced by nothing. Merged one level deep, with a per-key fallback so writing one key does not switch the others off. **A refusal from either premise rung is now an escalation that LEAVES (#555):** it goes through #274's door as a `stuck` post and a `blockers` row of class `decision`, keyed on the premise rather than its wording — so two premises on one PR are two rows and a restatement re-raises the first — and the report names the partition the fixer is about to act on, listing the findings the premise explains under *write no patch for these* against the independent half that still gets fixed. A declaration that named no findings is told it has stated no partition at all. Only the refusal announces; the ordinary declaration that runs before every fix pass still touches no network. **Both rungs rest on the declaration coming BEFORE the patch, and #560 makes ONE half of that checkable.** Every declaration is stamped with `git rev-parse HEAD` in the tree it was made from — read by `--premise` itself, never passed in, because the actor supplying the evidence would be the actor it is about — and a round names in `config_notes` any declaration stamped with the head of a LATER round, which is a premise declared once its own fix pass was written, committed and pushed. That is the shape lexray#1697 produced twice and it has no innocent explanation. Positive identification only: a stamp matching no round's head is silence, not an accusation, since an unpushed commit, an amend, a rebase and a stray checkout all land there. **The other half is not detectable here and the silence is a decision, not a gap.** A patch written into the working tree and not yet committed moves no `HEAD`, so its declaration's stamp is its own round's head — identical to the honest one. A dirty-tree flag was tried and accused any fixer whose checkout carried an unrelated modification; a porcelain fingerprint compared across the round and the declaration was tried and accused any fixer whose checkout a concurrent agent, an autosave or a background build touched in between, while missing a file that was already modified when the round ran. Hashing contents closes the second and not the first, because the ambiguity is in the inference and not the digest: a tree change cannot be attributed to a particular actor from evidence read in that actor's own environment, which is #622. `round_stop.premises` carries `retroactive`, `stamped` (how many declarations carried a head at all, a floor on checkability rather than a ceiling) and `wired` — whether THIS ROUND's invocation was handed a `--premise-file` path, which is what tells a cycle whose fixer skipped a declaration from a cycle in which the reader was never pointed at a register. `wired` does not establish that the fixer was given that path or could write it; `declared` is what says somebody reached the register. |
...
| `review_panel.budget` | #55's spend ceiling: `tokens_per_day`, `runs_per_day`, `tokens_per_round`, `tokens_per_pr`, `runs_per_pr`, `fleet_tokens_per_day`. **Every one `null` — no ceiling — which is what landing it did to every repo.** Checked against `GET /review/spend` *before any seat is dispatched*, and a repo at its ceiling is refused through the pre-flight's own machinery: printed, recorded with `reviewed: false` and a per-seat `ran: false`, posted to the PR, and carrying `stop_confident: false` so a budget stop cannot read as convergence. Tokens are input + output (`/review/stats`' `billable`); the run ceilings are what still binds where nothing was instrumented, and `runs_per_pr` is the one that binds a caller which renumbers its rounds. **`tokens_per_round` is the primary of the two per-PR token dials (#483)** — the unit of the budget is the round, because that is the unit of the work; `tokens_per_pr` is derived from it as `tokens_per_round × max_rounds` and a smaller value written there is refused rather than allowed to contradict it, and the allowance is released one round at a time so rounds 1-2 cannot eat the whole cycle's worth. `0` is a real value and means nothing may be spent; `--force` overrides none of them. Board-settable, and the board's value beats this file's — a ceiling a repo can raise from inside itself is decorative. |
...
`max_fix_growth`, `max_fix_growth_chars`, `min_fix_growth_chars`, `low_severity_fix_full_chars` and `low_severity_fix_lines`, all of which distinguish an absent key
...
before any seat is dispatched. Six numbers, all `null` by default:

| dial | measured over |
|---|---|
| `tokens_per_day` / `runs_per_day` | this repo, rolling `budget_window_hours` |
| `tokens_per_round` | this PR's whole life, against an allowance released one round at a time |
...
**The unit of the budget is the unit of the work (#483).** `tokens_per_round` is the
primary of the two per-PR token dials and `tokens_per_pr` is **derived** from it —
`tokens_per_round × max_rounds` — so the pair cannot be written to contradict each
other; a `tokens_per_pr` *below* that total is refused by name at resolve time, before
any board call. A flat per-PR total does not bound spend per unit of work, it bounds
...
gone wrong rather than a PR that has had enough attention — and a cycle whose rounds
each stayed inside the allowance can run every round the cap allows. Rounds are counted
