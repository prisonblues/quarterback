...
# has ultra, grok stops at xhigh), so they are listed per CLI rather than
# unioned. Per-MODEL support is narrower still and moves with the fleet
# (gpt-5.6-luna takes `max` but not `ultra`), so this only catches typos — the
# API rules on the model/effort pair and its sentence is surfaced verbatim.
# grok is the exception that proves the value: its CLI validates the level
# locally and exits before the turn starts, naming the four it takes.
