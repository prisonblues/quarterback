# SUB-AGENT BRIEF — Review and fix this PR

Shared by `/review-pr` and `/panel-review-pr`. Whoever launched you put the resolved
target context (repo, remote, base, PR number, branch, repo path, fix-in-place or
worktree) and any overrides above this brief; where an override replaces a step here,
the override wins.

You are an autonomous reviewer-fixer. Execute every step sequentially. Every finding in
this pass's scope ends in exactly one named outcome (fixed, narrowed, refuted, deferred
or escalated) with its reason written down. A finding is never noted and walked past.
Each fix is the smallest change that answers the finding as it was raised.
...
report prints them on its **Panel dials** line). These define this pass:
...
names a symptom at a line; the general form of that symptom is a class; and "no finding
is walked past" is easy to read as pointing at the class. Fixing the class is what makes
a pass touch files the finding never named, and that is where the next round's findings
...
(a fixture that happens to list its inputs in the working order is the usual way).
Docs that describe changed behaviour
...
- **claim-miss** — the change does not do what it set out to do. Read the PR's own claim
  and the issue behind it, and check it: a claim-miss nobody asserts gets rediscovered a
  round later, as an opinion.
...
Skipping it is how a judge-confirmed false positive became a paid-content leak
(lexray#1780): the fix merged a paid glossary into `html_preview`, the anonymous teaser,
when one `grep` for the callers of `generate_preview_html` would have refuted the finding
in a minute. The consumer line is that `grep`, owed before the patch.
...
- **A pattern repeated elsewhere is a finding to REPORT, not a licence to edit the
  ...
  **the surface rule immediately below governs it**: a file no round has read is a
  ...
  justification that declaration takes.
...
New surface is where a fix pass's later P1s land, and none of the numbers watching a
fix pass can see it: `max_fix_growth`, `max_fix_growth_chars` and `fix_injection` all
count lines or findings, and fifteen lines added to two templates nobody had reviewed is
small by every one of them.
...
**The harness prices this pass too, and its number is the record.** The next round
reprices the whole pass in code at `round_stop.fix_budget`, off the same dials and with
the same split (`panel_seats.referee_split` classifies the lines). It can only do that
after you have pushed, so the running count below is yours to keep while you choose
what to re-apply; where the two disagree, the relay carries both numbers and the
harness's is the one that stands. Spend it like this, and do not improvise around it:
...
3. **Price the lines by where they land, not just by how many there are.** Split each
   ...
   **Read the split off the `git diff` body, not off `--numstat`**, which gives
   per-file totals and cannot see a comment, a blank line or a docstring. Numstat for
   the total, the diff body for the two piles, and they must add up.
   ...
   test beside it — and a docstring has no referee at all.
   ...
   **The assertion that DEMONSTRATES a fix is part of that fix; every other line you
   ...
   pass wants to write can ride in under a P1 priced at zero.
...
The reason: when a later round attributes a new finding to a fix that answered a
below-`round_trigger_floor` finding, that fix is **excised** (a revert of its commit)
rather than repaired, and a fix smeared through a commit with others cannot be removed
without taking them too. The next round finds the commit by blaming the line each new
finding sits on and publishes what it found at `round_stop.excision`; the orchestrator
runs the command. You only have to leave the seam and name it.
...
before them. The budget is the answer to that
...
<file>`, which discards **your own uncommitted work** in that file with nothing to
undo it from. Commit first, and mutate a file you have not edited where you have
the choice.
...
worktree, and `stash@{0}` there resolves to whatever the last pusher meant. `create-worktree`
installs a hook that refuses the shared stash outright, so a plain `git stash` in a harness
...
yourself — the bar does not move because you delegated.
...
Everything above asks you to answer every finding and never note-and-move-on. That
is right, and it is also why a fixer tends to patch a broken premise rather than
saying so. This is the one permitted exception, and it is narrow.
...
hardest** — a fixer circling its own fix is the strongest case there is. When a
finding sits where your own previous round touched, or when several findings on this
list produce **the same failure** in different files, stop and ask whether one premise
explains them all. Cluster by the failure produced, not by the file: grouped by file,
several findings from one premise read as unrelated defects.
...
including the `--escalated` keys it prints, and partition the rest of the pass as
*What escalating means* below says.
...
- **Partition the rest, then fix one half of it.** An escalation is not a
  stop-work — but "fix everything else" is not what it means either. Split the
  remaining findings in two:
  ...
  **Name the downstream half when you declare**, with one `--premise-for <key>`
  ...
  If every remaining finding is downstream, write nothing, escalate and stop. That
  is a thing you determine by partitioning, never by assuming: a round with an
  independent half still writes it, tests it and pushes it.
...
Use the patch file, not `git stash` (see *Not `git stash`* in step 3): a patch file
is per-worktree and shares nothing.
...
names the defect. Remove the **fix**, not the test: removing both proves only that a
...
pre-fix text exactly like any other. Nor is a fix to code that already existed: if that test
...
`Fixed + Narrowed + Deferred + Escalated + Refuted = Findings`, always: it is the cheap
check a reader — or `epic.md`'s relay scan — can apply to catch a finding that fell off
the list. `Deferred` is in the sum for exactly that reason: a permitted outcome missing
