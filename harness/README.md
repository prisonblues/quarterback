...
README now points to `CHANGELOG.md` instead of carrying release history. That removes the case;
it does not remove the class. A
...
entries are written on the integration branch after the merge, by `scripts/release.py run`,
and by nothing else. A branch that writes them is writing a file every other branch also has
to write, at the same offset, so N such branches in flight is N-choose-2 conflicts **by
construction** — over nothing, since both entries are right and both belong. On 2026-08-23
that was three of six open pull requests, all `CONFLICTING`,
