...
2. Explain to the user, concisely (don't dump the README — summarise):
   ...
   - **Per-repo config:** each repo carries its own tracked `.harness-rules.sample` for policy,
     ...
     safe built-in defaults, and a repo with no file at all still works for every loop — except a
     panel review, which is REFUSED rather than run on defaults nobody chose. Tell the user what
     THIS repo resolves to (`harness_rules.py --json`) and which loops it has enabled.
3. List **every command**, both slash and CLI (CLI commands run from anywhere — the engine takes
   ...
   name under `~/source`. There is no central config: each repo carries its own rules file, and
   one that has none falls back to safe defaults, so the read-only commands work anywhere — bar
   `panel.py`, which refuses a repo whose rules nobody wrote. Epic run state is in
   `~/.local/state/loops/`.

   One rule worth stating: unattended runs (the timer) read `.harness-rules.sample` from the
   repo's **default branch**, not the working tree, so an upstream PR branch can't rewrite the
   policy governing its own review — and that includes the untracked per-box overlay, which is
