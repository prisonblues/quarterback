...
4. **Join the line, then ask for the verdict.** Work in the PR branch's checkout.
   ...
   **Do not assemble, number, stamp or tag anything.** There is nothing to run: the number is
   ...
   branch wrote the same file, so N of them in flight was N-choose-2 conflicts **by
   ...
   If you write to `CHANGELOG.md` anyway, `harness/githooks/pre-push` refuses the push and names
   `changelog.d/<issue>.<kind>.md` in the refusal, and the `generated release files are output`
   CI job refuses the pull request. Both are the answer, not an obstacle: obey the refusal, do
   not route around it.
...
- **A branch that writes to a file the release job generates** — the `generated release files
  are output` job (#122). `CHANGELOG.md` release entries are written on `$BASE` after the
  merge and by nothing else; a branch that edits them is refused here and by `pre-push`, with
