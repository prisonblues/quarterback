...
3. The **master triages each sub-issue for doability** — issues an agent can't actually do (e.g.
   ...
   could not rule at all reports `untriaged (no verdict: …)` / `untriaged (bad verdict: …)`, naming
   the cause: what the CLI said on stderr when it produced nothing at all (a denied tool permission,
   an unusable model pin), or what was wrong with the reply when it did answer (`no JSON in reply`,
   `malformed JSON`) — never the stderr of a run that replied, since a judge's warm-up chatter is
   not why its answer was unusable. Untriaged issues are also skipped on `--execute`, like blocked
   ones, so that reason is the only account of why one was passed over.
