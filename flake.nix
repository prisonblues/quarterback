    ...
    in
    {
      ...
      checks = forAllSystems (pkgs: {
        ...
        worktree-tests = pkgs.runCommand "quarterback-worktree-tests"
          ...
          # `release.py` loads `changelog_fragments.py` from beside itself, lazily, so
          # `guard` errors on a missing import without it. The file is declared here rather
          # than trusted to the import graph because the failure would arrive inside a
          # refusal path rather than at startup.
        ...
        prose-consistency-tests = pkgs.runCommand "quarterback-prose-consistency-tests"
          ...
          # `loops/docs/landing-hazards.md` points at are still jobs in it, under the ids and
