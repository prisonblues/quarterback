...
`--json-file` writes exactly the bytes `record_run` pipes to `qb`, so the note names the recovery rather than describing it: `qb record-review < <the payload>`, from any host that has one. That needs no queue, no retry daemon and no state that can go stale — the artefact already exists, and the board's own idempotency key means a replay joins the run rather than double-counting it.
