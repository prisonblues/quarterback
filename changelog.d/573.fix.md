...
* **A fetch that failed** was being thrown away with `|| true`, so nothing said the
  refs were older than the sweep. It is now reported — but as a fact about one remote
  and not as a veto. Remotes are fetched one at a time and the `!` line names which one
  did not answer and what git said about it; the question is still asked, because those
  refs are still there and still subtracted, exactly as `--no-fetch` has always been
  allowed to do. `fetch --all` exits non-zero if *any* remote fails, so collapsing them
  into one status meant a single permanently dead remote — a retired fork, a box that is
  off — refused the question on every merge for ever, on the hook path that never passes
  `--no-fetch`, leaving that machine with the hedge on every line and never the signal.
