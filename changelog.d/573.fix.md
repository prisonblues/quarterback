...
ninety times over. What is shared is the *answer*: three tests pin the tools to the
same query, the same grace window and — by running both against one checkout and
comparing what they say — the same verdict, because two tools disagreeing about whether
work exists elsewhere is worse than either being wrong on its own. The third exists
because the first two match strings in the two sources, and a literal being present in
a file is not evidence that it is on the live code path.

That budget is also why the walk is taken **once for the sweep**. `--not --remotes` has
to load and mark uninteresting every remote-tracking ref there is before it can answer,
and `qb-hook` wraps this in `timeout 25` and drops the whole report on overrun — so
ninety worktrees each paying for that walk is paid as silent loss of output rather than
as latency anybody notices. One repository-wide walk over `--branches` — `qb-doctor`'s
own call — names the candidates, and a second walk is paid only for the worktrees whose
branch tip that walk actually named. A branch carries stranded commits if and only if
its own tip is stranded, because anything reachable from a tip some remote ref reaches
is reached by that ref too.
