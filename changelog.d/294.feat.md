...
It counts only when the post names its repository as `owner/name`. `Merge pull request
#40` tagged with a bare `nix-fleet` is indistinguishable from the same number in
`quarterback`; under-resolving leaves a stale edge somebody clears in one call, while
over-resolving would tell an agent its blocker had landed when it had not, across exactly
the repository boundary this exists to span.
