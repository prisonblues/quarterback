...
findings. `—` is round 1, where the question does not arise; `?` is a round that was asked and
could not answer, which is never printed as `0` — a bolded claim about a fix pass, made from a
measurement that failed, is the flattering direction. A round that reviewed nothing prints
`not run` rather than `0 findings`, which would put the strongest convergence signal the block
can show against a round that never happened.
