...
that names something to **execute**, so it is opt-in, it is not a board dial — a dial
for a command line would be a way to run code on every box in the fleet by POSTing to
an API — and it runs only where the checkout is **already sitting on the PR's head**
with no uncommitted edits to tracked files. Where that holds, whoever put the branch
there has already accepted this code on this box; where it does not, the round says
so in its notes and keeps today's behaviour.
...
reads GitHub and has never heard of it. `local-fail` **vetoes**, which is the single
place a local answer is treated differently from the CI answer of the same shape —
CI's `FAIL` costs a round nothing precisely because the merge gate refuses on red
anyway, and for a local failure that second gate does not exist. A run that does not
finish inside `local_suite_timeout` is reported as not having finished, and vetoes;
it never becomes a pass, and it never quietly becomes a failure either.
