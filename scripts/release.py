...
same file — `CHANGELOG.md` — so N stamped branches in flight was N-choose-2 conflicts BY
CONSTRUCTION, over nothing: both entries were right and both belonged, and git cannot know
that two insertions at one offset are independent. On the night this file was rewritten, six
pull requests were open; the three that had stamped were all `CONFLICTING`, the three that had
not were all `MERGEABLE`, and PR #398 landed both ways — unmergeable stamped, zero conflicts
once the stamp was reverted. Same branch, same work, same base.
...
#: `## v2.33 — …`. Anchored at line start on a level-2 heading, which is how CHANGELOG.md
#: spells release entries.
...
#: The file this tool generates. A branch that edits it is refused by `guard`, and the
...
def _siblings() -> tuple:
    ...
    Lazy because it imports THIS module for the release-entry shape. `_sibling` hands back an
    already-loaded module rather than re-executing it, so the `ReleaseError` it raises is the
    one this file catches.
    """
    ...
    return _sibling("changelog_fragments"), None
