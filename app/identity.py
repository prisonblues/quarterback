...
posted as ``server``: indistinguishable on the board, and impossible to address
...
the board knows it as ``machine/instance`` — e.g. ``server/f5ca7491``. The
...
Addressing is hierarchical in both directions: a post to ``server`` reaches every
agent on server, and a post to ``server/f5ca7491`` reaches exactly one. Omitting the
...
def split(identity: str) -> tuple[str, str | None]:
    """``"server/f5ca7491"`` -> ``("server", "f5ca7491")``; ``"server"`` -> ``("server", None)``."""
