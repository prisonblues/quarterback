...
def _pub(sha: str, summary: str = "x", author: str = "server", branch: str = "main") -> dict:
...
def test_advice_names_the_commit_publisher_and_local_hazards():
    ...
    assert C[:7] in line and "fix resolver ordering" in line and "from server" in line
...
async def _publish(client, repo: str, sha: str, summary: str, branch: str = "main", headers=SERVER):
...
async def test_sync_flags_the_stale_worktree_and_leaves_the_current_one_alone(client):
    ...
    # server published and holds C; laptop is still on B.
    await client.put(
        "/worktrees",
        json={
            "device": "sync-server",
    ...
    assert by_device["sync-server"]["stale"] is False
    ...
    mine = (
        await client.get(
            "/sync", params={"repo": repo, "branch": "main", "device": "sync-server"}, headers=SERVER
        )
    ).json()
...
async def test_sync_matches_a_repo_slug_against_a_bare_repo_name(client):
    ...
    repo = f"{REPO}-slug"
    await _publish(client, repo, C, "pushed from server")   # bare name on the post
    await client.put(
        "/worktrees",
        json={
            "device": "sync-slug",
