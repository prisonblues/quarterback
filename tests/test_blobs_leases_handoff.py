...
async def test_blob_round_trip_and_idempotent(client):
    ...
    got = await client.get(f"/blob/{sha}", headers=SERVER)
...
async def test_lease_renew_guards(client):
    ...
    assert (
        await client.post("/lease/renew", json={"lease_id": lease_id}, headers=SERVER)
    ).status_code == 403
    # Owner can renew.
    assert (
...
async def test_lease_expires_then_peer_can_claim(client):
    ...
    assert (
        await client.post("/lease", json={"session": sess, "device": "zeu"}, headers=SERVER)
    ).status_code == 409

    await asyncio.sleep(1.2)  # lease lapses (simulates a crashed holder)

    # Now the peer can take over.
    took = await client.post("/lease", json={"session": sess, "device": "zeu"}, headers=SERVER)
    assert took.status_code == 200
    assert took.json()["holder"] == "server"
