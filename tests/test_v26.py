...
async def test_subagent_register_shows_active_then_ends(client):
    ...
    card = next(c for c in (await client.get("/sessions", headers=SERVER)).json() if c["session"] == ps)
    ...
    end = await client.post("/subagent/end", json={"parent_session": ps, "agent_id": "a1"}, headers=SERVER)
